...
export type EntityProfileData = {
  ...
  /** Optional structured verification metadata populated by admin review or a claimed business profile. */
  verificationMetadata?: {
    rbiRegistrationStatus: "confirmed" | "claimed" | "pending" | "not_found";
    rbiSourceUrl?: string;
    officialDomains: string[];
    claimedHandles: Array<{ platform: string; handle: string; status: "claimed" | "verified" | "pending"; claimedAt?: string }>;
    history: Array<{ date: string; event: string; status: string; source?: string }>;
  };
  grievance: {
    officerName: string;
    email: string;
