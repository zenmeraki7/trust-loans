...
import { useAdminLoanAppDatabase, useCreateAdminLoanApp } from "@/hooks/useAdminDashboards";

export default function AdminLoanAppDatabaseRoute() {
  const query = useAdminLoanAppDatabase();
  const createApp = useCreateAdminLoanApp();
  ...
  return (
    <AdminLoanAppDatabasePage
      data={query.data}
      createError={createApp.isError ? (createApp.error as Error).message : ""}
      isCreating={createApp.isPending}
      onCreateApp={(input) => createApp.mutateAsync(input)}
    />
  );
}
