...
export function EntityHero({ data }: { data: EntityProfileData }) {
  return <section className="rounded-3xl border border-slate-200 bg-white/85 p-6 shadow-sm md:p-8"><h1 className="text-2xl font-semibold text-slate-900 md:text-4xl">{data.displayName}</h1><div className="mt-2 flex flex-wrap gap-2"><RelationshipTypeBadge label={data.entityType} /><VerificationStatusBadge status={data.verificationStatus} /><RiskBadge level={data.riskSignalLevel} /></div><p className="mt-2 text-sm text-slate-600">Linked-app trust score avg: {data.averageLinkedAppTrustScore} • Linked apps: {data.totalLinkedApps} • Reviews across linked apps: {data.totalReviewsAcrossApps}</p><div className="mt-3 flex flex-wrap gap-2"><button className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white">View Linked Apps</button><button className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold">Submit Correction</button><button className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold">Claim This Entity</button><button className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold">Report Experience</button></div></section>;
}
