...
function ReportCompanyButton({ name }: { name: string }) {
  const [open, setOpen] = useState(false);
  return <>{<button type="button" onClick={() => setOpen(true)} className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-semibold text-rose-700">Report</button>}{open && <ReportEscalationModal entityName={name} entityType="nbfc" onClose={() => setOpen(false)} />}</>;
}
            ...
            {filteredItems.map((item) => (
              <article key={item.id} className="rounded-2xl border border-slate-200/60 bg-white p-5 shadow-sm">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <ReportCompanyButton name={item.displayName} />
                </div>
              </article>
            ))}
          </section>
