...
export default function SubmitLoanAppReviewPage({ slug }: { slug: string }) {
  ...
  if (!authChecked) {
    return <main className="min-h-screen bg-slate-50 p-6"><section className="mx-auto max-w-2xl rounded-2xl border bg-white p-8 text-center text-sm text-slate-600">Checking your account...</section></main>;
  }

  if (!isAuthenticated) {
    return (
      <main className="min-h-screen bg-slate-50 px-4 py-10 sm:px-6">
        <section className="mx-auto max-w-2xl rounded-2xl border border-blue-200 bg-white p-6 text-center shadow-sm sm:p-8">
          <h1 className="text-2xl font-bold text-slate-950">Create an account to share a review</h1>
          <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-slate-600">An account is required to submit an NBFC or loan-app review so we can protect the community from spam and follow up safely. Your public identity is still yours to choose.</p>
          <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
            <Link href={`/login?next=${encodeURIComponent(`/loan-apps/${slug}/submit-review`)}`} className="rounded-xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white">Log in to continue</Link>
            <Link href={`/signup?next=${encodeURIComponent(`/loan-apps/${slug}/submit-review`)}`} className="rounded-xl border border-slate-300 px-5 py-3 text-sm font-semibold text-slate-700">Create an account</Link>
          </div>
          <p className="mt-5 text-xs text-slate-500">Use Privacy controls in the review form to publish anonymously, show only a first name, or choose a custom display name.</p>
        </section>
      </main>
    );
  }
