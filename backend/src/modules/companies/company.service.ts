...
const buildProfile = (slug: string, apps: CompanyApp[], nbfc?: NbfcCompany | null) => {
  ...
  return {
    ...
    details: {
      ...
      businessType: nbfc?.businessType ?? first?.businessType ?? "",
      playStoreUrl: nbfc?.playStoreUrl ?? first?.playStoreUrl ?? "",
      appStoreUrl: nbfc?.appStoreUrl ?? first?.appStoreUrl ?? "",
      ...
      registrationNumber: nbfc?.rbiRegistrationNumber ?? first?.rbiRegistrationNumber ?? "",
      rbiRegistrationClaim: nbfc?.nbfcRegistrationClaim ?? first?.claimedNbfcPartner ?? "",
      rbiRegistrationVerifiedAt: (nbfc?.rbiRegistrationVerifiedAt ?? first?.rbiRegistrationVerifiedAt)?.toISOString().slice(0, 10) ?? "",
      rbiRegistrationSourceUrl: nbfc?.rbiRegistrationSourceUrl ?? first?.rbiRegistrationSourceUrl ?? "",
      associatedRegulatedEntity: nbfc?.associatedRegulatedEntity ?? first?.associatedRegulatedEntity ?? first?.claimedNbfcPartner ?? "",
      interestRateRange: nbfc?.interestRateRange ?? first?.interestRateRange ?? "",
      processingFees: nbfc?.processingFees ?? first?.processingFees ?? "",
      latePaymentCharges: nbfc?.latePaymentCharges ?? first?.latePaymentCharges ?? "",
      loanTenure: nbfc?.loanTenure ?? first?.loanTenure ?? "",
      privacyDisclosure: nbfc?.privacyDisclosure ?? first?.privacyDisclosure ?? "",
      contactAccessDisclosure: nbfc?.contactAccessDisclosure ?? first?.contactAccessDisclosure ?? "",
      recoveryPracticeInfo: nbfc?.recoveryPracticeInfo ?? first?.recoveryPracticeInfo ?? "",
      knownComplaintCategories: nbfc?.knownComplaintCategories?.length ? nbfc.knownComplaintCategories : first?.knownComplaintCategories ?? [],
      publicWarningLabels: nbfc?.publicWarningLabels?.length ? nbfc.publicWarningLabels : first?.publicWarningLabels ?? [],
      dataSource: nbfc?.dataSource ?? first?.dataSource ?? "Public metadata and user-submitted reports",
      lastReviewedAt: (nbfc?.lastReviewedAt ?? first?.lastReviewedAt ?? nbfc?.updatedAt ?? first?.updatedAt)?.toISOString().slice(0, 10) ?? "",
    ...
    grievance: {
      officerName: nbfc?.grievanceOfficerName ?? "",
...
export const companyService = {
  ...
  async create(input: CreateCompanyProfileInput) {
    ...
    const nbfc = await prisma.nbfcCompany.create({
      data: {
        ...
        grievanceOfficerName: input.grievanceOfficerName,
  ...
  async enrich(slug: string, input: EnrichCompanyProfileInput) {
    ...
    const data = {
      ...
      ...(input.grievanceOfficerName !== undefined ? { grievanceOfficerName: input.grievanceOfficerName } : {}),
      ...
      ...(input.rbiRegistrationNumber !== undefined ? { rbiRegistrationNumber: input.rbiRegistrationNumber } : {}),
      ...(input.rbiRegistrationVerifiedAt !== undefined ? { rbiRegistrationVerifiedAt: input.rbiRegistrationVerifiedAt } : {}),
      ...(input.rbiRegistrationSourceUrl !== undefined ? { rbiRegistrationSourceUrl: input.rbiRegistrationSourceUrl } : {}),
      ...(input.associatedRegulatedEntity !== undefined ? { associatedRegulatedEntity: input.associatedRegulatedEntity } : {}),
      ...(input.interestRateRange !== undefined ? { interestRateRange: input.interestRateRange } : {}),
      ...(input.processingFees !== undefined ? { processingFees: input.processingFees } : {}),
      ...(input.latePaymentCharges !== undefined ? { latePaymentCharges: input.latePaymentCharges } : {}),
      ...(input.loanTenure !== undefined ? { loanTenure: input.loanTenure } : {}),
      ...(input.privacyDisclosure !== undefined ? { privacyDisclosure: input.privacyDisclosure } : {}),
      ...(input.contactAccessDisclosure !== undefined ? { contactAccessDisclosure: input.contactAccessDisclosure } : {}),
      ...(input.recoveryPracticeInfo !== undefined ? { recoveryPracticeInfo: input.recoveryPracticeInfo } : {}),
      ...(input.knownComplaintCategories !== undefined ? { knownComplaintCategories: input.knownComplaintCategories } : {}),
      ...(input.publicWarningLabels !== undefined ? { publicWarningLabels: input.publicWarningLabels } : {}),
